...
export class PaymentService {
  ...
  createOrder(cartId: number, shipToAddress: AddressDto) {
    return this.http
      .post<ResponseDto<any>>('Order/CreateOrder', { cartId, shipToAddress })
      .pipe(
        tap((res) => {
          if (res.isSuccessed) {
            const options: any = {
              ...
              currency: 'INR',
              name: 'CartVerse', //Your Company Name
              ...
              handler: (paymentRes: any) => {
                ...
                this.updatePayment(
                  ...
                  this.router.navigateByUrl(
                    '/orders/detail/' + res.data.orderId,
                  );
                });
              },
              prefill: {
  ...
  private updatePayment(orderId: string, paymentId: string, signature: string) {
    return this.http.post('Payment/update-payment', {
      orderId,
      paymentId,
      signature,
    });
